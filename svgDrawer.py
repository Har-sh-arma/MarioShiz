...
def appendToFile(ax:float, ay:float, az:float, vx:float, vy:float, vz:float, dt:float):
    # s_x = (math.floor((vx*dt + (ax*dt*dt)/2)*100))/100
    # s_y = (math.floor((vy*dt + (ay*dt*dt)/2)*100))/100
    s_z = (math.floor((vz*dt + (az*dt*dt)/2)*100))/100
    # vx_n = (math.floor((vx+ ax*dt)*100))/100
    # vy_n = (math.floor((vy+ ay*dt)*100))/100
    vz_n = (math.floor((vy+ ay*dt)*100))/100
